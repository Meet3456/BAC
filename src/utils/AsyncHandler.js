...
// answer : The asyncHandler is a utility function( A higher order function) that takes a function as an argument and returns a new function. The  new function is an asynchronous function that calls the original function (passed to it) and catches any errors that it throws. If an error is thrown, the new function sends a JSON response with the error message and a status code of 500. This allows you to write route handlers that use async/await syntax without having to worry about catching errors manually.
